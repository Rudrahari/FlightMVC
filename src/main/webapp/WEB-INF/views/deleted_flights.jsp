...
<%
  List<Flight> publishedFlights=(List<Flight>) session.getAttribute("deletedFlight");
  List<AirlinesFare> publishedFlightsFare=(List<AirlinesFare>) session.getAttribute("deletedFlightsFare");
  List<Airlines> publishedFlightsSeats=(List<Airlines>) session.getAttribute("deletedFlightsSeats");
%>
    <h1>Deleted Flight Details</h1>
    ...
    <% for(int i=0;i<publishedFlights.size();i++){ %>
        <li class="flight-item">
            ...
            <div class="action-buttons">
                ...
                <button class="delete" onclick="addFlightToService(<%= publishedFlights.get(i).getId() %>)">Add Flight to Service Line</button>
            </div>
        </li>
       <% } %>
    </ul>
...
<script>
   /*  function viewPassengerList(flightId) {
    ...
    function addFlightToService(flightId) {
        var shouldDelete = confirm("Do you want to reintroduce this flight to service?");
        if (shouldDelete) {
            window.location.href = "addFlightService?id=" + flightId;
        }
    }
